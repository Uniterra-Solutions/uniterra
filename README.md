...
| --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TDD development workflow** (`uniterra-plan` → `uniterra-implement` → `uniterra-simplify` / `uniterra-review`) | Plan → clarify requirements + design interactively → write `prd.md` / `design.md` / `acceptance.md` → confirm the plan with the user → review them with 3 parallel agents (feasibility / over-engineering / verifiable acceptance), hand the failing axes' issues to a repair agent, and re-review only the axes that failed (a passed axis is never re-dispatched) → implement: write ALL failing property tests, decompose into a task list, run a batched / full-parallel workflow of subagents → review (property-based: extract each branch's pre/post-conditions + invariants into a formal spec table, prove them with >10,000-run PBT, shrink counterexamples into structured error reports, then a fixer repairs each and a main agent aggregates by severity + user impact) and/or simplify (over-engineering checklist, behaviour-preserving; the plan's design is an authoritative constraint). Property testing blocks the bugs a known invariant would introduce, during development |
