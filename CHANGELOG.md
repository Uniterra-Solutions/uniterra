...
- **Machine-readable update progress with a durable completion record (issue #15)** (`packages/uniterra-cli/src/update-progress.ts`, `packages/uniterra-desktop/src/update-progress.ts`, `packages/uniterra-desktop/src/update-launch.ts`). `uniterra setup` and `uniterra update` now emit NDJSON progress events on stdout beside the untouched human output: every event line starts with the fixed sentinel `@@uniterra `, the key set is CLOSED (`v`/`run`/`seq`/`at`/`event`/`stage`/`status`/`message`), `seq` starts at 0 and increments by exactly one per run, and no human line may start with the sentinel — the prefix compare alone partitions the two streams. The human half stays exactly one init notice, one start/end line per EXECUTED stage and one run summary. With `UNITERRA_UPDATE_PROGRESS_FILE` set, the same events are mirrored line for line into a durable record; the desktop reads that record on its next boot and shows the last UNCONSUMED terminal run exactly once (`pendingSummary` → `progressDialogContent`, then a consumption marker, so the following boot is silent). That record is the only channel that survives an update: Update Now quits the app and the updater then replaces the CLI itself, so no process is left to be told the outcome. The updater hand-off lives in its own module, `src/update-launch.ts` — the one spawn site, detached and `unref`ed, which is also what puts the record path into the child's environment.
- **The Skill Market built-in (issue #27)** (`vendor/dsh-plugins/dsh-skill-market/`, `packages/uniterra-desktop/src/builtin.ts`). `QQ-M/dsh-skill-market` is vendored at its pinned main-branch commit (there is no npm release) and registered as a `kind: 'vendor'` built-in, so it rides the existing copy + staleness + retirement provisioning. It adds a "Skill Market" page to the settings UI that searches GitHub for skill repositories and installs them with one click into dsh's USER SKILL ROOT (`<DSH_HOME>/skills` — the root the model's `skill` tool reads), served by the host half as a same-origin JSON API. Five LOCAL PATCHES are recorded in `vendor/dsh-plugins/VENDOR.md` and the vendor-plugins doc row, each with a removal condition: the bare `schemastery` import (no dsh profile resolves the scoped one), a pure `resolveSkillRoot()` (the author's `/root/.dsh/skills` default and his token-file path are gone), the dropped `@deepseek-ai/dsh-client-runtime` inject row/peer (absent from the pinned family), READMEs that no longer name author paths, and the containment fix recorded under Fixed below.
- **`planSurfaces` — the launch-surface plan of one run (issue #28)** (`packages/uniterra-cli/src/install-logic.ts`). Over every (command, open, dryRun, platform, destination) combination a run yields at most one Electron surface, exactly one iff `open` && !`dryRun`, ZERO browser surfaces, and the Electron target is a path rather than a URL — the machine-checkable half of "one update leaves exactly the desktop window".

### Changed

- **The bundled `dsh-skill-creator` skill writes to the user skills root by default** (`packages/uniterra-skills/src/skills/dsh-skill-creator/SKILL.md`). It used to say "the narrowest root that matches who needs it" with no default; a personal skill now defaults to `<dshHome>/skills` (`~/.dsh/skills`, rank 400 — the list the `skill` tool and the user both see), a project root is for a repository-bound skill only, and a new pitfall covers filing a personal habit inside one repository. The catalog description carries the default and stays inside the 500-character window.
- **The project docs follow the release**: `AGENTS.md` (the desktop and CLI responsibility-map entries), `docs/modules/uniterra-cli.md` (a new Progress Stream section with the export table, the sentinel and the cross-package wire contract, the durable record, and the single-surface decision), `docs/modules/uniterra-desktop.md` (the two new modules, the boot step that reports the last update once, and the rebuilt staleness rule), `docs/modules/uniterra-skills.md` and `docs/testing.md` (the new suites plus the rule that a property asserts the STANDARD's literal, never the constant under test).

### Fixed

- **The update sentinel matches the plan of record** (`packages/uniterra-cli/src/update-progress.ts`, `packages/uniterra-desktop/src/update-progress.ts`). Both packages shipped `@uniterra ` while the PRD fixes `@@uniterra `, and the shipped tests took their expected bytes from the module constant they were testing, so they passed on either spelling. Both codecs, both golden vectors and two new property suites now pin the documented bytes.
- **A one-click skill install copies repository bytes only** (`vendor/dsh-plugins/dsh-skill-market/index.js`). `copyTree()` followed symlinks, so a market repository shipping `leak -> ~/.ssh/id_ed25519` made the install copy bytes from OUTSIDE the downloaded tree into the model-readable skill root. An entry is now copied by its own kind: anything that is not a regular file is skipped.
- **Built-in provisioning is total and foreign-preserving** (`packages/uniterra-desktop/src/builtin.ts`). A manifest body that is not a JSON object (`null`, an empty string, `42`, an array) is left alone instead of throwing out of the boot pass; a `bundles` value in ANY shape — the flat list this pass writes, a name→flag map another tool writes, a nested list — keeps every row it names, so a foreign profile no longer loses its rows; and staleness is judged by the implementation files the SOURCE ships, every one of which must exist in the installed copy, so a copy that lost a runtime file to an interrupted update is healed on the next boot.
- **The dsh spawn contract cannot be re-opened through an option value** (`packages/uniterra-desktop/src/dsh-process.ts`). A profile beginning with `--` would have been parsed by dsh as a flag, re-enabling the OS browser handoff the shell suppresses; such a value is now attached to its option (`--profile=<value>`), which every parser reads as a value.
- **The acceptance table names the suite that exists** (`.plan/13-09-2026/issues-15-27-28/ACCEPTANCE.md`). All twelve evidence rows cited `pnpm --filter @uniterra-solutions/uniterra-cli test`, which matches no package: it printed "No projects matched the filters" and exited 0, so the gate could never fail. The rows now name the real package.

## [0.17.1] — 2026-09-13

### Added

- **Two bundled authoring skills: `dsh-skill-creator` and `dsh-prompt-writer`** (`packages/uniterra-skills/src/skills/dsh-skill-creator/SKILL.md`, `packages/uniterra-skills/src/skills/dsh-prompt-writer/SKILL.md`). `dsh-skill-creator` teaches the dsh skill contract end to end: the six roots a skill can live in and their precedence (project `.dsh/skills` → `.agents/skills` → custom dirs → user `<dshHome>/skills` → the agents home → the read-only bundled root), kebab-case naming plus the two supported layouts (`<name>/SKILL.md`, `<name>.md`), the frontmatter fields (`name`, `description`, `whenToUse`, `disable-model-invocation`, `user-invocable`) and the visibility combination each produces, body-writing principles, resource layout, and how to verify discovery. `dsh-prompt-writer` turns a request into one self-contained executor order: the six beats (recon with real tools → settle every decision in one `ask_user_question` call → write the complete order → deliver it as a single four-backtick fenced block → hand it to the executor → verify with real tools), the canonical order structure, the dsh dispatch specifics (a `subagent` sees no conversation, so the order carries everything; `run_workflow` args stay tiny) and `references/pbt-first-dev-mode.md` with the tests-first rules every software order bakes in. Both are provisioned from `SKILL_NAMES` and asserted by `packages/uniterra-skills/test/provision.test.mts`.
- **Retired workflow capsules are healed out of already-provisioned profiles** (`packages/uniterra-desktop/src/builtin.ts`). `RETIRED_WORKFLOW_CAPSULES` names the capsule files uniterra itself provisioned for skills that left the bundle, and `ensureWorkflowCapsules` removes them from `$DSH_HOME/workflows` unconditionally — before its `skillsDir` check — so a profile heals even when the bundle no longer resolves; the copy loop also skips those basenames, and the `true` return now reports a removal as well as a write. Pinned by `packages/uniterra-desktop/test/workflow-capsule-provision-pbt.test.mjs` (including a deterministic case for an undefined or missing skills dir) and `test/builtin-pbt.test.mjs`.

### Changed

- **The bundled review workflow is the only persisted `dsh.workflow` capsule** (`packages/uniterra-skills/scripts/build-workflow-capsules.mjs`): the builder now emits `src/skills/uniterra-review/workflows/review.workflow.json` alone — its prompt still composed from the skill's `references/*.md`, its provenance timestamp still fixed, so a rebuild stays byte-identical — and the `implementSource()` / `simplifySource()` builders plus the `extractPrompt()` helper they needed are gone.
- **The registry, the test suites and the docs follow the new set** (`packages/uniterra-skills/src/index.ts`, `test/`, `AGENTS.md`, `README.md`, `docs/`). `SKILL_NAMES` now provisions eight skills — `uniterra-pbt-debugging`, `uniterra-review`, `manage-agents-md`, `manage-git-repo`, `project-documentation`, `uniterra-qa`, `dsh-skill-creator`, `dsh-prompt-writer` — and the capsule-contract, orchestration-PBT and deterministic-regression suites cover the review capsule only, with every REVIEW invariant (single pass, the standard documents verbatim in both prompts, `compliance` pass-through, a failed fixer surfacing as `failed`) preserved. Every live reference to the retired pipeline is gone; the retired-name lists and this changelog are the only remaining mentions.
- **The two rule documents — `AGENTS.md` and `docs/conventions.md` — are compressed under a 32,000-byte combined budget** (19,641 bytes after the change). They are the always-loaded orientation surface: AGENTS.md's per-package narratives became one-line responsibility-map entries that link the module doc which already owns the detail, and conventions.md's wide rule tables (whose prettier pipe-padding alone cost roughly 14 KB) became compact bullet lists plus a new Workflow Capsules section. Every rule survived, and both files now state the budget with the `wc -c` check, so the next edit cannot silently regrow them.

### Removed

- **The `uniterra-plan` / `uniterra-implement` / `uniterra-simplify` pipeline skills and their capsules** (`packages/uniterra-skills/src/skills/uniterra-plan/`, `uniterra-implement/`, `uniterra-simplify/`, their `scripts/init_plan.mjs` / `scripts/init_task.mjs` scaffolding CLIs, `workflows/implement.workflow.json`, `workflows/simplify.workflow.json`, and `test/scripts.test.mts`). The three phase skills were stages of ONE workflow rather than independently useful capabilities, and what they carried now ships as the two authoring skills above plus `uniterra-review`. The names joined `RETIRED_SKILL_NAMES` (`packages/uniterra-skills/src/index.ts`), so a profile provisioned by an earlier version drops the three skill directories on the next boot, and `RETIRED_WORKFLOW_CAPSULES` drops the two capsules from `$DSH_HOME/workflows`.

## [0.17.0] — 2026-09-13

**The DeepSeek Harness moves to the 0.1.5-rc.2 family, which raises the session-log format to v3 — the session layer of this upgrade is ONE-WAY. Back up `~/.dsh` before the first 0.1.5 launch** ([setup.md](docs/setup.md#upgrading-a-012-era-profile-session-v3-is-one-way)). A resumed pre-upgrade conversation also keeps the tool roster it was created with, so start a NEW conversation after upgrading ([setup.md](docs/setup.md#start-a-new-conversation-after-upgrading)).

### Added

- **Turn-completion notifications: a finished turn raises one native OS notification carrying the session title and how the turn ended** (`packages/uniterra-desktop`, `src/notifications.ts` / `src/turn-observer.ts` / `src/dsh-observer.ts`). The shell owns the whole path — this replaces the retired `dsh-notifier` plugin rather than extending it, so there is no bundle row to reconcile. The preference lives in the profile's `.uniterra.json` (`notifications.turnComplete`, **default ON**) and is driven by a native application-menu checkbox; the file is shared with the optional-plugin toggle, so a write is additive and an illegible user file is never rewritten. Only user-facing sessions are followed — an intermediate subagent turn never notifies — and one `turn/end` frame produces exactly one notice however often a follow stream replays it (the stream replays its opening window on every reconnect by design). **Fail-soft by construction:** no auth cookie, a refused socket, a malformed frame or a dead runtime all degrade to silence and a log line, never an error, and nothing blocks boot. The transport deliberately never opens the `$events` waterfall stream — its `approval/request` and `user-questions/request` frames stall host settlement process-wide unless the subscriber answers each one. Guarantees pinned by `test/turn-notification-pbt.test.mjs`, `test/turn-notification-preference-pbt.test.mjs`, `test/turn-observer-pbt.test.mjs` and `test/turn-observer-transport.test.mjs`, plus a LIVE harness (`scripts/verify-turn-notification/run.sh`) that boots the bundled dsh in a throwaway `DSH_HOME` and drives the real transport — because a wire seam read out of the vendored sources is a claim about the shipped runtime, not a fact.
- **`dsh-ego-browser` is vendored and registered as a built-in** (`vendor/dsh-plugins/ego-browser`, `packages/uniterra-desktop/src/builtin.ts`): 32 `ego_*` browser-automation tools plus a realtime watch panel (live SSE screencast, direct mouse interaction, download capture, human-verification notice). It is vendored **without a local patch** — the deliberate, documented exception to the "vendor only to customize" policy: npm's only ever release peers on `@deepseek-ai/dsh-client-runtime`, a package the pinned family no longer ships, so an npm import cannot resolve, while this pin declares `dsh.engines.dsh >=0.1.2-rc.1` and self-inserts its own Loader row. Upstream verbatim, **MIT**; its bare `schemastery` runtime dependency is installed as a profile runtime dep (`PROFILE_RUNTIME_DEPS`).
- **A live verification harness for the turn observer** (`scripts/verify-turn-notification/`): re-runnable, boots the bundled dsh in a throwaway `DSH_HOME` and asserts the real transport end to end (root-token cookie exchange, `session/list`, `session/follow`). `AGENTS.md` now requires it after any `dsh-observer.ts` change or dsh pin bump.

### Changed

- **Every `@deepseek-ai/*` dependency is pinned exact at 0.1.5-rc.2 and `vendor/dsh-harness` is re-pinned to the `dsh-v0.1.5-rc.2` tag** (commit `fb2c4b9e`) — the desktop devDependency, the provider's peers, and the vendored tree stay consistent, and the pin ledger records the status of each local divergence. The two pnpm patches are re-derived against the 0.1.5 tarballs: same alias-only semantics, renamed to the 0.1.5-rc.2 targets, each now carrying a `REMOVAL CONDITION` header.
- **Session logs move to format v3, and the upgrade is one-way** (`docs/setup.md`, `docs/architecture.md`). The first 0.1.5 launch migrates a pre-upgrade session in place — header and events rewritten, `agentPreset: "code"` → `ptc`, the original log left untouched beside the new `session.v3.jsonl.zstd` — and a 0.1.2-era app can no longer read it. Resuming an upgraded session in the older app appends to the frozen pre-upgrade file while 0.1.5 keeps writing the v3 file: two logs, one session id, silently diverging, with no downgrade migration in either direction. The docs carry the backup command and the rollback guidance.
- **A resumed pre-upgrade conversation keeps the tool set it was created with** (`docs/setup.md`). dsh rebuilds a live session from its own log, so the request replayed to the model carries the roster recorded when that session was created: measured at **75 tools with 17 retired `browser_*` and no `ego_*`** for a resumed pre-upgrade session versus **93 tools with 32 `ego_*`** for a new one. An old conversation that decides to browse calls a tool that no longer exists; everything else (chat, files, bash, git, `computer_*`, workflow) is unaffected, and a new conversation is always current. This is a session-log property of dsh, not something the app can migrate.
- **The desktop shell is adapted to every 0.1.5 surface it owns** (`packages/uniterra-desktop/src/`). `ensureProfileInitialized` asks the CLI for a boot-free `--profile web --dump-config` before provisioning, so a machine that has never run dsh gets a real profile instead of the bare shipped template — without it the first boot ended with no built-ins and `run_workflow` unavailable. `ensureWorkflowCapsules` provisions the three pipeline capsules into `$DSH_HOME/workflows` byte-idempotently. `startDsh` now passes `--no-open` (dsh otherwise hands its readiness URL to the operating system's default browser, and the shell IS this app's browser) and fails fast when the child dies before readiness — racing the exit against the 60 s readiness window and cancelling the losing wait, which is also what took the spawn suite from 60.1 s to 2.1 s. `resolveDshCliPath`'s Windows `.pnpm`-store scan now takes the NEWEST `@deepseek-ai+dsh@*` shard; `readdir` order had resolved a stale 0.1.2-rc.1.
- **`preset-compat` converges on 0.1.5: only the settings half still needs it** (`packages/uniterra-desktop/src/preset-compat.ts`). Session HEADERS migrate natively — the installed `v0 → v1 → v2 → v3` catalog rewrites `agentPreset: "code"` → `ptc` in every old header and `agent-preset/selected` event, and deliberately leaves a native v3 header alone — so resuming an old session no longer needs the shim. The settings row `agent-presets.default: code` is NOT migrated by upstream and is read verbatim as the roster's default preset id, so a session naming no preset still fails until the row exists; the user-preset shim is kept, never overwrites an existing file, and carries its removal condition. The measurement is recorded in the module doc.
- **The developer toolchain is upgraded** (`package.json`, `eslint.config.mjs`, `tsconfig.base.json`): eslint + `@eslint/js` 9 → 10, typescript 5.9 → 6.0.3 (deliberately not 7.x), `typescript-eslint` 8.70.x, `@types/node` 22.20.2, `lint-staged` 16 → 17. Rule intent and thresholds are unchanged — `strictTypeChecked`, `--max-warnings 0`, the same `vendor/`/`lib/`/`dist/` ignores — with no `@ts-ignore`/`@ts-expect-error` escape hatch added anywhere.
- **The vendored plugin pins move with the family** (`vendor/dsh-plugins/`): `@dsh-external/workflow` → v0.1.4 (metadata-only over v0.1.3 — `lib/engine.js` byte-identical, only the version stamps move; its five content patches stay under the same package version), `dsh-deep-whale` → v0.1.2, and `dsh-shortcuts` gets two further client seams re-planted on the native 0.1.5 API (`ctx.sidebarRight.toggleExpanded()` for the details toggle, the transient `assistant/live-chunk` entry for the streaming copy fallback) with the four 0.1.2 patches intact — all 34 shortcuts individually re-verified with zero missing seams.
- **`dsh-better-sidebar` moves to the native sidebar API and the built-in set is re-verified end to end** (`packages/uniterra-desktop/src/builtin.ts`, `docs/modules/vendor-plugins.md`). Every npm built-in is evaluated on 0.1.5 with a recorded verdict covering load plus one key action; no entry is silently disabled — a failure is an explicit recorded degradation or a follow-up issue.
- **Docs and specs synced with the code** (`AGENTS.md`, `README.md`, `docs/`). The turn-notification feature is documented for the first time (module doc, README, architecture diagrams and flow, testing lanes, conventions), the desktop module doc's staleness rule is corrected (vendored AND workspace copies are compared by `package.json` version OR a content fingerprint of the shared implementation files — the text claimed version-only for workspace copies), the vendor update policy's pinned-family reference is corrected to 0.1.5-rc.2, and the project-structure doc's package count, vendor-plugin table and script rows match the tree again.

### Removed

- **`packages/uniterra-systemprompt` and its `@earendil-works/pi-coding-agent` (pi-ai) dependency are gone** — with the root `tsconfig.json` project reference, the CI step that ran its test, its documentation page and its rows in the docs and `AGENTS.md`. The working-rules injection it implemented is deliberately not reimplemented here.
- **Three npm built-ins are retired into the 0.1.5 core instead of shipping twice** (`packages/uniterra-desktop/src/builtin.ts`): `dsh-file-upload` (the web app now carries its own `id: file-upload` Loader row, and 0.1.5's loader REJECTS a duplicate entry id — a profile still listing the plugin cannot boot 0.1.5 at all, `duplicate loader entry id: file-upload`), the browser-automation plugin (superseded by the vendored `dsh-ego-browser`), and the subagent model picker (the native `subagent` tool selects `provider`/`model` per delegation). Each stays declared `retired: true` in the same registry, so an already-provisioned profile is healed by removal on its next ensure pass — bundle rows, `dependencies` entries and installed copies — idempotently, and user-installed copies are never touched. Their `minimumReleaseAgeExclude` entries and docs rows are gone.

### Fixed

- **Both provider serializers handle the `file` content block dsh 0.1.5 added** (`packages/uniterra-provider`, `src/serialize-chat.ts` / `src/serialize-response.ts`). The block-type switches are exhaustive, so the new variant was a build error (`switch-exhaustiveness-check: Cases not matched: "file"`) before it could ever be a wire problem; each serializer now projects it explicitly through the harness's `fileHandleText` helper with no blanket `default`, so a future variant still fails the build by name. Regression net: `test/config-compatibility.test.mjs` pins that a pre-upgrade provider configuration still loads (unknown keys tolerated, missing keys defaulted), so an upgraded profile keeps its gateway and model rows.
- **The CLI's update-CLI failure now carries its cause** (`packages/uniterra-cli`, `src/cli.ts`) — the npm error survives the rethrow instead of collapsing into a bare wrapper message.
...
- **`uniterra-implement` now executes against the acceptance criteria as the specification** (`packages/uniterra-skills`, `src/skills/uniterra-implement/`, `scripts/build-workflow-capsules.mjs`). The red suite is written as **test as spec** — every acceptance line maps to at least one test and every test traces back to a requirement — and the decomposition step gained a **seam rule**: every cross-task seam (an interface, a serialized shape, an event one task emits and another consumes) must already be pinned by a test or be promoted to a requirement / acceptance line, with seam-sharing tasks dispatched in the same batch (each mocking the seam) or in separate batches with the provider first. Each task brief now carries its own requirements + acceptance + allocated tests (`## Requirements & Acceptance` in `init_task.mjs`'s template), so a subagent holds only its own slice of the spec. The `implement` capsule returns per-task evidence — `{ status: 'done', agents, reports }` or `{ status: 'failed', batch, reports }`, each report a `{ id, ...structured }` entry in dispatch order (a child that returned nothing stays visible as `{ id, failed: true }`) — and the skill gained a reconciliation step: every requirement must be claimed by some report's `satisfied_requirements`, and every one-line `deviations` entry is adjudicated (a spec problem goes back to the user and into `acceptance.md`; an interface change is checked against the affected tasks' tests). The design-deviation rule is gone: the fixed rules now forbid weakening, narrowing or deleting an allocated test (a test believed wrong stays RED and is recorded), which is the only remaining spec-issue channel.
- **`uniterra-review` reviews against the requirements of record — requirement as standard — and produces the compliance table itself** (`packages/uniterra-skills`, `src/skills/uniterra-review/`, `scripts/build-workflow-capsules.mjs`). `run_workflow('review', { task, standard })` takes an optional `standard = { requirements, acceptance }` (repo-relative paths to the plan's `prd.md` + `acceptance.md`); both documents are read at run time and inlined VERBATIM into the review AND the fixer prompt under `## Standard (authoritative — the requirements + acceptance of record)` — documents in, main-agent narrative out, so the anti-pollution boundary is structural rather than advisory. The review agent audits every requirement line (acceptance line → the test it names → exists / passes / non-hollow) and returns `compliance` rows (`pass` / `fail` / `missing` / `contradiction`, an optional additive schema field) plus the three instrument findings — **coverage gap**, **hollow test**, **spec contradiction** — while the three-layer PBT verification stays as the standard-external axis; the fixer receives the standard too and never repairs against a requirement (a conflicting report is refused, explained, and reported as `failed` under the existing semantics); the main agent aggregates the compliance summary (requirements X/Y, acceptance M/N) next to the severity report, still trusting the reported evidence instead of re-running the tests. With no `standard` the review behaves exactly as before (pure code modelling), which the capsule contract tests pin. The same authority shift reaches `uniterra-simplify`: its binding context is now `requirements` + `acceptance`, the legacy `design` block is optional, and its absence is never a reason to simplify away required machinery.
