...
| [dsh-skill-creator](#dsh-skill-creator)           | Create/add a dsh skill (建立技能/技能創建/新增 skill)                           | Pick the root (project / user / bundled + duplicate-name precedence) → kebab-case name + layout → frontmatter contract → a body that changes behaviour → verify discovery with the `skill` tool                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Teaches the dsh skill contract: the six roots a skill can live in and their precedence
(project `.dsh/skills` → `.agents/skills` → custom dirs → user `<dshHome>/skills` → agents
home → the read-only bundled root), kebab-case naming plus the two supported layouts
