...
| [uniterra-review](#uniterra-review)               | Adversarial review / hunt for bugs / run the review phase (with a review scope)     | goal + context → review (extract each branch's pre/post-conditions + invariants into a formal spec table → prove with >10k-run PBT → shrink counterexamples into structured error reports) → fixer repairs each → main agent aggregates by severity (critical/medium/low) + user impact |
...
Property-based adversarial review — usable standalone, no plan required. Assemble a goal + context (requirements / design / acceptance) + task, then a `workflow` (review → fix → aggregate): the **review agent** reads every business module in scope, discovers the repo's test + property-testing conventions (never assumes a framework), traverses every conditional branch and extracts its pre/post-conditions + invariant into a formal specification table, writes a property test per invariant in the repo's conventional test location (descriptive invariant-based name, never a finding id), and executes them with an iteration budget **> 10,000 runs**. It shrinks every counterexample to its minimal failing input and wraps it as a structured error report (id, severity, file, line, invariant, input, expected/actual, test). Only confirmed counterexamples are reported. The **fixer agent** repairs each reported branch so its property test passes, re-runs the counterexample green, and returns a diff + result + explanation. The **main agent** aggregates every counterexample + fix by severity (**critical / medium / low**) and explicitly lists which business logic is wrong, why, and the user impact, plus whether each was fixed. The workflow is a **single pass** (no re-review loop) — because the PBT runs **> 10,000 iterations per invariant**, the outcome is a statistically strong (near-formal) proof, so it is trusted after one review; the fixer only re-runs each counterexample to confirm it is green. Severity: **critical** = wrong results / data loss / a security hole / a core invariant that never holds; **medium** = fails on an edge/error path or a non-core invariant; **low** = a confirmed but non-blocking counterexample (rare — style/naming nits are not reported). A `pass` verdict means no critical/medium counterexample remains open.
