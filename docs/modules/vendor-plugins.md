...
| --------------- | ---------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dsh-shortcuts` | dsh-shortcuts          | `bf392410…`   | 34 keyboard shortcuts, one-click recording, macOS-first. **LOCAL PATCH (2026-09-05, 0.1.2-rc.1 API migration):** four client seams the new family removed/relocated were fixed in `lib/client.js` (new session via `sessions.create` + `open`; copy-last-message via `binding.eventSource`; focus-composer via the `[role="textbox"]` contentEditable seat; theme toggle only flips the built-in light/dark pair, never dropping a registered skin). Every seam is pinned by the LOCAL conformance suite: `packages/uniterra-desktop/test/dsh-shortcuts-*.test.mjs` (plugin contract + dsh-api oracle over `vendor/dsh-harness` + behavior/PBT) and `scripts/verify-dsh-shortcuts-smoke/run.sh` (real dsh web boot) — see the ledger in `vendor/dsh-plugins/VENDOR.md`                         |
| `dsh-workflow`  | @dsh-external/workflow | `804e4c38…`   | KodaX-parity dynamic multi-agent workflow layer: persists workflows as `.workflow.json` capsules, exposes `workflow_list` / `run_workflow` / `workflow_manage`. Approval gate keys on the session sandbox mode (danger-full-access → ungated; read-only / workspace-write → user approval popup); background launches are owned by the DSH job system once they outlive the tool step. Carries several content patches under the same package version — see the `dsh-workflow` row's **LOCAL PATCH** notes in `vendor/dsh-plugins/VENDOR.md`; the desktop propagates them by content fingerprint (the 2026-09-05 patch adds a session-API compatibility layer: `Session#events` was removed upstream in 0.1.2-rc.1, so the engine reads `snapshotEvents()` when the legacy accessor is absent) |
...
- Vendored plugin customized in place → record the divergence + pending-upstream note as a **LOCAL PATCH** in the `VENDOR.md` pin-ledger row, update this table's row, and keep the plugin's pinned-family conformance suite green (contract/dsh-api oracle/behavior tests + the real-boot smoke — every dsh seam the patch uses must be pinned; see `packages/uniterra-desktop/test/dsh-shortcuts-*.test.mjs` as the template).
