...
| `uniterra-review`        | Property-based adversarial review: goal + context → review (extract each branch's invariants into a formal spec table → write all tests in one pass → prove with >10,000-run PBT in a background job → shrink counterexamples into structured error reports) → fixer repairs each and reports back → main agent aggregates by severity + user impact (never re-running the tests) |
