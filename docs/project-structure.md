...
| Skill dir                | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `uniterra-plan`          | Planning phase: clarify requirements + design interactively → write prd.md / design.md / acceptance.md → confirm the plan with the user → SINGLE parallel review (3 agents: the main agent applies returned issues, may re-run the review fresh; no repair agent, no re-review loop)                                                                                                                                                                                                                                                                                                                |
| `uniterra-implement`     | PBT-first execution: write ALL failing PBTs → decompose into a task list (scripts/init_task.mjs scaffolds each brief under .dsh/<YYYYMMDD-HHmmss>/<project>/<task>.md with a per-project task.json; the capsule inlines each brief into the subagent prompt) → batched / full-parallel workflow of subagents                                                                                                                                                                                                                                                                                        |
...
| `uniterra-review`        | Property-based adversarial review (whole-business-model PBT): scope → review (the review agent gets ONLY the scope — no main-agent framing, so it isn't biased; models the whole business logic + lifecycle — every operation, every state — into a formal spec table of state / transition / lifecycle / data / security invariants, brute-forces random operation sequences with >10,000-run PBT in a background job, shrinks counterexamples into structured error reports) → fixer repairs each and reports back → main agent aggregates by severity + user impact (never re-running the tests) |
