...
| `uniterra-review`        | Property-based adversarial review: goal + context → review (extract each branch's invariants into a formal spec table → prove with >10,000-run PBT → shrink counterexamples into structured error reports) → fixer repairs each → main agent aggregates by severity + user impact |
