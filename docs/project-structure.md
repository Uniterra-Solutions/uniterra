...
| `uniterra-implement`     | PBT-first execution: write ALL failing PBTs → decompose into a task list (scripts/init_task.mjs scaffolds each brief under .dsh/<YYYYMMDD>/<task>/; the capsule inlines each brief into the subagent prompt) → batched / full-parallel workflow of subagents                                                                                                                                                                                                                                                                                                 |
...
| `uniterra-review`        | Property-based adversarial review: scope → review (the review agent gets ONLY the scope — no main-agent framing, so it isn't biased; extract each branch's invariants into a formal spec table, AND derive security invariants from the security checklist so logic security is PBT-verified too → write all tests in one pass → prove with >10,000-run PBT in a background job → shrink counterexamples into structured error reports) → fixer repairs each and reports back → main agent aggregates by severity + user impact (never re-running the tests) |
