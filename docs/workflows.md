...
5. Run `uniterra-review` (property-based adversarial: the review agent reads the business modules, extracts every conditional branch's pre/post-conditions + invariants into a formal spec table, writes a property test per invariant and runs >10,000 iterations, then shrinks each counterexample into a structured error report — file, line, input, expected/actual; a fixer repairs each and a main agent aggregates by severity critical/medium/low, stating which logic is wrong, why, and the user impact) and/or `uniterra-simplify` (over-engineering checklist, behaviour-preserving; the plan's design is an authoritative constraint). The review is proven sound when no counterexample remains.
