...
5. Run `uniterra-review` (property-based adversarial: the review agent reads every business module in one pass, extracts each conditional branch's pre/post-conditions + invariants into a formal spec table, writes ALL the property tests in one pass then runs them together in a background job with >10,000 iterations, and shrinks each counterexample into a structured error report — file, line, input, expected/actual; the fixer repairs each and reports back to the main agent, which aggregates by severity critical/medium/low, stating which logic is wrong, why, and the user impact — without ever re-running the tests) and/or `uniterra-simplify` (over-engineering checklist, behaviour-preserving; the plan's design is an authoritative constraint). The review is proven sound when no counterexample remains.
