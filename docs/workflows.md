...
3. Run the plan's review workflow — three parallel review agents (feasibility, over-engineering, verifiable acceptance), then a repair agent applies the failing axes' issues to the docs itself, and only the axes that failed the previous round are re-reviewed (a passed axis is never re-dispatched, so the review-agent count shrinks from 3 toward 0) — until all pass. Call `run_workflow('plan-review', { prd_dir, design_dir, acceptance_dir, maxRounds? })` (the persisted `plan-review` capsule; no script to copy).
4. Load `uniterra-implement`: write ALL failing property tests (red phase), decompose into a task list, call `run_workflow('implement', { tasks })` for independent tasks (full-parallel) or `run_workflow('implement', { batches })` for overlapping ones (serial batches of parallel tasks); confirm the full suite is green.
5. Run `uniterra-review` (property-based adversarial: the review agent reads every business module in one pass, extracts each conditional branch's pre/post-conditions + invariants into a formal spec table, AND derives security invariants from the security checklist so logic security is PBT-verified too, writes ALL the property tests in one pass then runs them together in a background job with >10,000 iterations, and shrinks each counterexample into a structured error report — file, line, input, expected/actual; the fixer repairs each, pins it with a deterministic unit regression test (concrete minimal input — no RNG), and reports back to the main agent, which aggregates by severity critical/medium/low, stating which logic is wrong, why, and the user impact — without ever re-running the tests) and/or `uniterra-simplify` (over-engineering checklist, behaviour-preserving; the plan's design is an authoritative constraint). The review is proven sound when no counterexample remains.
