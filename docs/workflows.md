...
4. Load `uniterra-implement`: write ALL failing property tests (red phase), decompose into a task list (write each task's brief to `.dsh/tasks/<id>.md` and pass `{ id, name, promptFile }` per task — keep the `args` JSON tiny so the tool call is never corrupted), call `run_workflow('implement', { tasks })` for independent tasks (full-parallel) or `run_workflow('implement', { batches })` for overlapping ones (serial batches of parallel tasks); confirm the full suite is green.
