/**
 * Copy dictionaries for the Uniterra settings section. Product copy is Chinese
 * first (the dsh web convention); English mirrors it.
 */

export const zh = {
  nav: 'Uniterra',
  intro:
    '配置 Uniterra 网关：API 密钥、网关地址、接口协议与模型列表。模型发现只列出支持 chat 接口的模型。',
  keyInput: 'API 密钥',
  keyPlaceholder: '粘贴令牌；留空保持已存密钥不变',
  keyStored: '已配置（不回显）',
  keyMissing: '未配置',
  keyEnvLocked: '由启动环境提供（只读）',
  baseUrl: '网关地址（含 /v1 前缀）',
  baseUrlPlaceholder: 'http://gw.local:3000/v1',
  apiProtocol: '默认接口协议',
  apiChatCompletions: 'chat-completions',
  apiResponses: 'responses',
  apiHint: '未指定协议的模型使用默认协议；每个模型可在高级设置中单独覆盖。',
  models: '模型',
  modelsEmpty: '暂无模型：点「添加模型」手动新增，或「获取模型」从网关拉取。',
  clearModels: '清空',
  addModel: '添加模型',
  removeModel: '删除该模型',
  modelAdvanced: '高级设置（上下文 / 输出上限 / 协议）',
  modelId: '模型 ID',
  modelName: '显示名称',
  modelApi: '接口协议（默认）',
  modelApiUseDefault: '跟随默认协议',
  modelImageInput: '视觉（图片输入）',
  contextWindow: '上下文窗口',
  maxTokens: '输出上限',
  modelReasoning: '思考等级',
  defaultEffort: '预设思考等级（切换模式时自动选择）',
  modelIdRequired: '模型 ID 不能为空',
  modelIdDuplicate: '模型 ID 重复',
  capacityInvalid: '容量需为正数，可用 K/M 缩写（如 128K、1M）',
  fetchModels: '获取模型',
  fetching: '正在询问网关…',
  fetchEmpty: '网关没有列出可用的 chat 模型（embedding / rerank / ranker 已过滤）。',
  fetchTitle: '选择要添加的模型',
  fetchAdopt: '添加所选',
  fetchCancel: '取消',
  updateParams: '从models.dev获取模型信息',
  paramsFetching: '正在查询 models.dev…',
  paramsTitle: '模型参数（来自 models.dev）',
  paramsSummary: '匹配 {matched} 个 · 未匹配 {unmatched} 个',
  paramsUnmatched: '未匹配（保持原值）',
  paramsProvider: '选择数据来源供应商',
  officialMark: '官方',
  paramsOverwrite: '应用（覆盖现有值）',
  paramsFillBlank: '仅填空白字段',
  paramsApplied: '已更新模型信息',
  paramsNoModels: '没有可查询的模型：先添加模型或从网关获取。',
  proxyToggle: '代理',
  proxyUrl: '代理地址',
  apply: '保存',
  applying: '正在保存…',
  saved: '已保存。',
  loadFailed: '加载失败',
  nsNotRegistered: 'llm-uniterra: 设置命名空间未注册（插件行是否已加载？）',
  retry: '重试',
  readOnly: '当前设置源只读，无法保存。',
  modelHint:
    '默认只列出 /chat/completions 接口支持的模型；不支持该接口的模型请手动添加。模型可单独选择 responses 协议。该配置可在 settings.yaml 的 llm-uniterra: 段用 modelExcludePatterns 调整。',
} as const;

export type UniterraKey = keyof typeof zh;

export const en: Record<UniterraKey, string> = {
  nav: 'Uniterra',
  intro:
    'Configure the Uniterra gateway: API key, gateway base URL, wire protocol, and model list. Discovery lists chat-capable models only.',
  keyInput: 'API key',
  keyPlaceholder: 'Paste the token; leave blank to keep the stored key',
  keyStored: 'Configured (never echoed)',
  keyMissing: 'Not configured',
  keyEnvLocked: 'Provided by the launch environment (read-only)',
  baseUrl: 'Gateway base URL (including /v1)',
  baseUrlPlaceholder: 'http://gw.local:3000/v1',
  apiProtocol: 'Default wire protocol',
  apiChatCompletions: 'chat-completions',
  apiResponses: 'responses',
  apiHint:
    'Models without their own protocol setting use the default; each model can override it under Advanced.',
  models: 'Models',
  modelsEmpty: 'No models yet: add one by hand, or fetch the list from the gateway.',
  clearModels: 'Clear',
  addModel: 'Add model',
  removeModel: 'Remove this model',
  modelAdvanced: 'Advanced (context window / max output / protocol)',
  modelId: 'Model ID',
  modelName: 'Display name',
  modelApi: 'Wire protocol (default)',
  modelApiUseDefault: 'Follow the default protocol',
  modelImageInput: 'Vision (image input)',
  contextWindow: 'Context window',
  maxTokens: 'Max output tokens',
  modelReasoning: 'Reasoning efforts',
  defaultEffort: 'Default reasoning effort (auto-selected on mode switch)',
  modelIdRequired: 'Model id is required',
  modelIdDuplicate: 'Duplicate model id',
  capacityInvalid: 'Capacity must be a positive number; K/M suffix allowed (e.g. 128K, 1M)',
  fetchModels: 'Fetch models',
  fetching: 'Asking the gateway…',
  fetchEmpty:
    'The gateway listed no chat-capable models (embedding / rerank / ranker filtered out).',
  fetchTitle: 'Choose models to add',
  fetchAdopt: 'Add selected',
  fetchCancel: 'Cancel',
  updateParams: 'Fetch model info from models.dev',
  paramsFetching: 'Querying models.dev…',
  paramsTitle: 'Model parameters (from models.dev)',
  paramsSummary: '{matched} matched · {unmatched} unmatched',
  paramsUnmatched: 'No match (values kept)',
  paramsProvider: 'Choose the data provider',
  officialMark: 'Official',
  paramsOverwrite: 'Apply (overwrite existing)',
  paramsFillBlank: 'Fill blank fields only',
  paramsApplied: 'Model info updated',
  paramsNoModels: 'No models to look up: add one or fetch from the gateway first.',
  proxyToggle: 'Proxy',
  proxyUrl: 'Proxy URL',
  apply: 'Save',
  applying: 'Saving…',
  saved: 'Saved.',
  loadFailed: 'Load failed',
  nsNotRegistered:
    'llm-uniterra: the settings namespace is not registered (is the plugin row loaded?)',
  retry: 'Retry',
  readOnly: 'The active settings source is read-only; nothing can be saved.',
  modelHint:
    'By default only models supported by the /chat/completions endpoint are listed; models without that support must be added manually. Models may select the responses protocol individually. Tune modelExcludePatterns in the llm-uniterra: settings section.',
};
