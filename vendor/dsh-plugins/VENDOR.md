...
| `dsh-skill-market` | `QQ-M/dsh-skill-market` | `8fa51ed574c10f978df85798cc48fbebc8d5d406` | (main @ `8fa51ed5…`, 2026-08-18, v0.1.0, **MIT**) "Skill Market" in the settings page: searches GitHub for skill repos and installs the skills with one click into dsh's USER SKILL ROOT (`<DSH_HOME>/skills`, the root the model's `skill` tool reads), served by the host half as a same-origin JSON API (`webServer` prefix route `/skill-market/api/*`); the browser half is a hand-written `__ModuleLoader__` bundle (no build step). Vendored at the **main-branch commit** (there is no npm release, and the only tag, `v0.1.0` → `96cd6909…`, is OLDER than main). **Trimmed** to the runtime/doc set — `index.js` + `client.js` + `package.json` + `cordis.patch.yml` + `LICENSE` + `README.md` + `README.zh.md`; the upstream `.gitignore` is NOT vendored. **LOCAL PATCHES (2026-09-13, uniterra integration):** (1) the host half statically imported the scoped `@deepseek-ai/schemastery`, which NO dsh profile resolves (verified in a throwaway `DSH_HOME`: `ERR_MODULE_NOT_FOUND`), while the BARE `schemastery` is already linked in the profile via `PROFILE_RUNTIME_DEPS` — the import is now the bare package, the way the vendored ego-browser plugin does it (no repository dependency added); (2) `installDir` defaulted to the author's own `/root/.dsh/skills` (config-schema default) and the shipped `cordis.patch.yml` row hardcoded that path PLUS a `githubTokenFile` inside the author's work tree — the copy now exports a pure `resolveSkillRoot({ explicit, dshHome, home })` used by BOTH the Config default and `apply()` (an explicit configuration wins, else the LIVE `DSH_HOME`, else `<home>/.dsh/skills`; always an absolute path) and ships NO config block, so the plugin's own defaults apply and no token file is read unless the user configures one; (3) `package.json` dropped its `@deepseek-ai/dsh-client-runtime` row from `dsh.client.inject` AND from `peerDependencies` — that package does not exist in the pinned 0.1.5-rc.2 family (the same LOCAL PATCH `dsh-shortcuts` carries), while `@deepseek-ai/dsh-client-locale` and `@deepseek-ai/dsh-client-ui-settings` do and are kept; (4) the two READMEs' config examples/tables no longer name the author's paths; (5) the installer's `copyTree()` copies an entry by its OWN kind — a symlink (or any other non-regular file) is SKIPPED instead of followed, so a market repository shipping `leak -> ~/.ssh/id_ed25519` can no longer make a one-click install copy bytes from OUTSIDE the downloaded tree into the model-readable skill root. **Pending upstream (`QQ-M/dsh-skill-market`):** republish against the pinned family — import a schema package a dsh profile resolves, default `installDir` to `<DSH_HOME>/skills`, and declare no removed inject row/peer. **REMOVAL CONDITION:** drop this vendored copy in favour of an npm import (or an unpatched vendored re-pin) as soon as such an upstream release exists; until then it stays vendored FOR these five patches. Pinned by `packages/uniterra-desktop/test/skill-market-*.test.mjs` (VENDOR-PIN / VENDOR-LEDGER / NO-AUTHOR-PATH / INJECT-ROWS) and the provisioning PBT. |
